...
struct StateInner {
    ...
    width_independent_heights: bool,
...
impl ListState {
    ...
    pub fn new(item_count: usize, alignment: ListAlignment, overdraw: Pixels) -> Self {
        let this = Self(Rc::new(RefCell::new(StateInner {
            ...
            width_independent_heights: false,
            pending_scroll: None,
    ...
    /// Declare that item heights do not depend on the list's width, so a
    /// width change keeps every measured size instead of invalidating it.
    ///
    /// By default a width change wipes all cached heights and re-measures
    /// every item, which is required when rows can reflow at a different
    /// width. Lists whose rows truncate or size from a fixed breakpoint keep
    /// their exact heights, so opting in makes a width change cost one O(n)
    /// tree walk instead of re-rendering every row: a per-frame layout-width
    /// animation stays cheap. Visible rows still re-render each prepaint, so
    /// a width-dependent row re-measures as soon as it scrolls into view.
    pub fn width_independent_heights(self) -> Self {
        self.0.borrow_mut().width_independent_heights = true;
        self
    }
...
impl Element for List {
    ...
    fn prepaint(
        ...
        {
            // Lists that declared width-independent heights keep every
            // measured size: the re-armed measure pass then reuses the cached
            // sizes in an O(n) walk instead of re-rendering each row, so a
            // per-frame layout-width animation never re-measures the feed.
            // remeasure_items and splice still downgrade their items to
            // Unmeasured, and visible rows still re-render every prepaint.
            if !state.width_independent_heights {
...
mod test {
    ...
    #[gpui::test]
    fn test_width_independent_heights_retain_sizes_across_width_change(cx: &mut TestAppContext) {
        let cx = cx.add_empty_window();
        ...
        let state = ListState::new(ITEMS, crate::ListAlignment::Top, px(0.))
            .measure_all()
        ...
        impl Render for TestView {
            fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl IntoElement {
                ...
                let render_count = self.1.clone();
                list(state, move |_, _, _| {
        ...
        let view = cx.update(|_, cx| cx.new(|_| TestView(state.clone(), render_count.clone())));

        // First draw at width 100: the measure-all pass renders every item
        // once, then the visible two render again for the frame.
        cx.draw(point(px(0.), px(0.)), size(px(100.), px(60.)), |_, _| {
            view.clone().into_any_element()
        });
        assert_eq!(render_count.get(), ITEMS + 2);
        let max_offset = state.max_offset_for_scrollbar().y;
        assert_eq!(max_offset, px(ITEMS as f32 * ITEM_HEIGHT - 60.));

        // A same-width frame re-renders only the visible rows.
        cx.draw(point(px(0.), px(0.)), size(px(100.), px(60.)), |_, _| {
            view.clone().into_any_element()
        });
        let steady_frame_renders = render_count.get() - (ITEMS + 2);
        assert_eq!(steady_frame_renders, 2);

        // Scroll halfway, then change the width: the retained heights must
        // keep the extent and the scroll anchor exact while the re-armed
        // measure pass reuses the cached sizes instead of re-rendering every
        // off-screen row.
        state.scroll_to(gpui::ListOffset {
            item_ix: 50,
            offset_in_item: px(0.),
        ...
        let before_width_change = render_count.get();

        cx.draw(point(px(0.), px(0.)), size(px(200.), px(60.)), |_, _| {
            view.clone().into_any_element()
        });

        assert_eq!(state.max_offset_for_scrollbar().y, max_offset);
        assert_eq!(state.logical_scroll_top().item_ix, 50);
        let width_change_renders = render_count.get() - before_width_change;
        assert!(
            width_change_renders <= steady_frame_renders + 2,
            "width change re-rendered {width_change_renders} rows, expected only the visible ones"
        );
    }

    #[gpui::test]
    fn test_width_change_without_the_flag_rerenders_every_row(cx: &mut TestAppContext) {
        let cx = cx.add_empty_window();
        ...
        let render_count = Rc::new(Cell::new(0usize));
        let state = ListState::new(ITEMS, crate::ListAlignment::Top, px(0.)).measure_all();
        ...
        // Without the declaration the width change invalidates every height,
        // so the re-armed measure pass must re-render and re-measure each
        // row at the new width within the same prepaint.
        cx.draw(point(px(0.), px(0.)), size(px(200.), px(60.)), |_, _| {
            view.into_any_element()
        });
        ...
        let width_change_renders = render_count.get() - before_width_change;
        assert!(
            width_change_renders >= ITEMS,
            "width change only re-rendered {width_change_renders} rows, expected every item"
        );
    }
