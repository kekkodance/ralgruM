...
impl DirectXAtlas {
    ...
    pub(crate) fn get_texture_view(
        ...
        lock.texture(id)
            .map_or([None], |texture| texture.view.clone())
    }
    ...
    fn get_or_insert_with<'a>(
        ...
        if let Some(tile) = lock.tiles_by_key.get(key) {
            ...
            let texture = lock
                ...
                .expect("newly allocated atlas texture is missing");
...
mod tests {
    ...
    #[test]
    fn test_stale_texture_id_returns_an_empty_view() {
        let Some(atlas) = create_atlas() else {
            return;
        };
        let key = make_image_key(4);
        ...
        assert!(atlas.get_texture_view(tile.texture_id)[0].is_none());
    }
}
