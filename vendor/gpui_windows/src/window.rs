...
pub struct WindowsWindowState {
    ...
    /// Tray popups use the swapchain alpha channel for their transparent margins.
    pub is_popup: bool,
...
pub(crate) struct WindowsWindowInner {
    ...
    pub(crate) app_owns_titlebar_drag: bool,
...
impl WindowsWindowInner {
    fn new(context: &mut WindowCreateContext, hwnd: HWND, cs: &CREATESTRUCTW) -> Result<Rc<Self>> {
        ...
        Ok(Rc::new(Self {
            ...
            app_owns_titlebar_drag: context.app_owns_titlebar_drag,
...
impl WindowsWindow {
    pub(crate) fn new(
        ...
        let mut context = WindowCreateContext {
            ...
            handle,
            hide_title_bar,
            app_owns_titlebar_drag: params.app_owns_titlebar_drag,
            display,
            ...
            platform_window_handle,
            appearance,
            is_popup: params.kind == WindowKind::PopUp,
            disable_direct_composition,
            directx_devices,
            invalidate_devices,
            draw_coordinator,
        ...
        if should_suppress_popup_dwm_chrome(&params.kind) {
            suppress_popup_dwm_border(hwnd);
            suppress_popup_dwm_non_client_rendering(hwnd);
        }
...
fn transparent_composition_state(is_popup: bool) -> u32 {
    if is_popup { 0 } else { 2 }
}

fn should_suppress_popup_dwm_chrome(kind: &WindowKind) -> bool {
    kind == &WindowKind::PopUp
}

fn suppress_popup_dwm_border(hwnd: HWND) {
...
fn suppress_popup_dwm_non_client_rendering(hwnd: HWND) {
...
#[cfg(test)]
mod popup_border_tests {
    use super::{WindowKind, should_suppress_popup_dwm_chrome, transparent_composition_state};

    #[test]
    fn popup_border_suppression_is_scoped_to_popup_windows() {
        assert!(should_suppress_popup_dwm_chrome(&WindowKind::PopUp));
        assert!(!should_suppress_popup_dwm_chrome(&WindowKind::Normal));
        assert!(!should_suppress_popup_dwm_chrome(&WindowKind::Dialog));
    }

    #[test]
    fn popup_dwm_non_client_suppression_is_scoped_to_the_same_popup_path() {
        assert!(should_suppress_popup_dwm_chrome(&WindowKind::PopUp));
        assert!(!should_suppress_popup_dwm_chrome(&WindowKind::Normal));
        assert!(!should_suppress_popup_dwm_chrome(&WindowKind::Dialog));
    }
    ...
    #[test]
    fn popup_transparency_uses_swapchain_alpha_without_a_native_gradient() {
        assert_eq!(transparent_composition_state(true), 0);
        assert_eq!(transparent_composition_state(false), 2);
    }
}
        ...
        match background_appearance {
            ...
            WindowBackgroundAppearance::Transparent => {
                set_window_composition_attribute(
                    hwnd,
                    None,
                    transparent_composition_state(self.state.is_popup),
                );
            }
