...
use crate::{
    ActiveTheme as _, ElementExt as _, FocusTrapElement as _, IconName, Root, Sizable as _,
...
impl Dialog {
    ...
    /// Sets the closing animation state and epoch for the dialog overlay.
    pub fn closing(mut self, closing: bool, epoch: u64) -> Self {
        self.props.closing = closing;
        self.props.close_epoch = epoch;
        self
    }
...
impl RenderOnce for Dialog {
    fn render(mut self, window: &mut Window, cx: &mut App) -> impl IntoElement {
                    ...
                    .child(
                        v_flex()
                            ...
                            .on_prepaint(move |bounds, window, cx| {
                                // Record the measured dialog frame so app overlays
                                // can test whether a trigger lives inside the modal.
                                Root::update(window, cx, |root, _, _| {
                                    root.set_dialog_bounds(layer_ix, bounds);
                                });
                            })
                            .with_animation(
                                "dialog-frame-in",
        ...
        let overlay_element: gpui::AnyElement = if self.props.closing {
            let close_animation = Animation::new(*ANIMATION_DURATION)
                .with_easing(cubic_bezier(0.32, 0.72, 0., 1.));
            overlay_element
                .with_animation(
                    ("dialog-fade-out", self.props.close_epoch),
                    close_animation,
                    move |this, delta| this.opacity(1.0 - delta),
                )
                .into_any_element()
        } else {
            overlay_element
