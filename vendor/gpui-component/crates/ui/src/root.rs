...
pub struct Root {
    ...
    pub(crate) dialog_bounds: Vec<Bounds<Pixels>>,
...
impl Root {
    ...
    pub(crate) fn set_dialog_bounds(&mut self, layer_ix: usize, bounds: Bounds<Pixels>) {
        if self.dialog_bounds.len() <= layer_ix {
            self.dialog_bounds
                .resize(layer_ix + 1, Bounds::default());
        }
        self.dialog_bounds[layer_ix] = bounds;
    }

    /// Bounds of the topmost active dialog from the last prepaint, if known.
    ///
    /// Returns None when no dialog is open or the dialog has not painted yet.
    /// Callers should treat unknown bounds as outside so stale background
    /// triggers stay suppressed until the dialog frame is measured.
    pub fn top_dialog_bounds(window: &Window, cx: &App) -> Option<Bounds<Pixels>> {
        let root = window.root::<Root>()??;
        let bounds = root.read(cx).dialog_bounds.last().copied()?;
        ...
        F: Fn(Dialog, &mut Window, &mut App) -> Dialog + 'static,
    {
        ...
        // Keep a placeholder so top_dialog_bounds reports unknown until the
        // new dialog frame paints and records its measured bounds.
        self.dialog_bounds.push(Bounds::default());
    ...
    fn close_dialog_internal(&mut self) -> Option<FocusHandle> {
        ...
        self.dialog_bounds.truncate(self.active_dialogs.len());
        handle
    }
