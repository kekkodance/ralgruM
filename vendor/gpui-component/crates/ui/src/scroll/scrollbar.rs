...
impl Scrollbar {
    ...
    fn style_for_normal(&self, cx: &App) -> (Background, Hsla, Hsla, Pixels, Pixels, Pixels) {
        ...
        // Hover keeps the original thin resting thumb and only grows while
        // dragging or hovering the thumb itself. Showing the wider active
        // thumb at rest made every vertical bar look fatter.
        let (width, inset, radius) = match scrollbar_show {
            ScrollbarShow::Always => (THUMB_ACTIVE_WIDTH, THUMB_ACTIVE_INSET, THUMB_ACTIVE_RADIUS),
            ScrollbarShow::Scrolling | ScrollbarShow::Hover => {
    ...
    fn style_for_idle(&self, cx: &App) -> (Background, Hsla, Hsla, Pixels, Pixels, Pixels) {
        let scrollbar_show = self.scrollbar_show.unwrap_or(cx.theme().scrollbar_show);
        // Same thin resting width as normal so hidden Hover bars fade back
        // to the original size instead of the wider active size.
        let (width, inset, radius) = match scrollbar_show {
            ScrollbarShow::Always => (THUMB_ACTIVE_WIDTH, THUMB_ACTIVE_INSET, THUMB_ACTIVE_RADIUS),
            ScrollbarShow::Scrolling | ScrollbarShow::Hover => {
                (THUMB_WIDTH, THUMB_INSET, THUMB_RADIUS)
