...
use gpui::{
    Animation, AnimationExt as _, AnyElement, App, ClickEvent, Context, DismissEvent, Div, Edges,
    ElementId, Entity, EventEmitter, FocusHandle, Focusable, InteractiveElement, IntoElement,
    KeyBinding, Length, ParentElement, Render, RenderOnce, SharedString,
    StatefulInteractiveElement, StyleRefinement, Styled, Window, anchored, deferred, div,
    ease_out_quint, prelude::FluentBuilder, px, rems,
};
use rust_i18n::t;
...
struct SelectOptions {
    ...
    disabled: bool,
    appearance: bool,
    menu_map: Option<Box<dyn Fn(Div) -> AnyElement + 'static>>,
}
            ...
            let _ = this.update_in(cx, |this, _, cx| {
                if this.closing && this.close_epoch == epoch {
                    ...
                    GlobalState::global_mut(cx)
                        .unregister_deferred_popover(&this.state.focus_handle);
                    cx.notify();
                }
            });
        })
                                    ...
                                    let surface = v_flex()
                                        ...
                                        .child(
                                            List::new(&self.state.list)
                                                ...
                                                .paddings(Edges::all(px(6.))),
                                        );
                                    let menu = match self.menu_map.as_ref() {
                                        Some(map) => map(surface),
                                        None => surface.into_any_element(),
                                    };
                                    let closing = self.closing;
                                    let animation_id = if closing {
                                        ElementId::from(("select-menu-exit", self.close_epoch))
                                    } else {
                                        ElementId::from((
                                            "select-menu-entry",
                                            cx.entity().entity_id(),
    ...
    <D::Item as SearchableListItem>::Value: PartialEq + Clone,
{
    ...
    /// Wrap the dropdown menu surface after default popover chrome is applied.
    pub fn map_menu(mut self, f: impl Fn(Div) -> AnyElement + 'static) -> Self {
        self.options.menu_map = Some(Box::new(f));
        self
    }
}
...
mod tests {
    ...
    #[test]
    fn select_wraps_menu_enter_and_exit_animation() {
