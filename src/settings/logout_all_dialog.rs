...
mod tests {
    #[test]
    fn logout_all_dialog_asks_for_confirmation_before_logging_out() {
        let source = include_str!("logout_all_dialog.rs");
        let production = &source[..source.find("#[cfg(test)]").unwrap()];
        assert!(production.contains("Log out of all services?"));
        assert!(
            production
                .contains("This signs you out of Murglar, SoundCloud, and Deezer on this device.")
        );
        assert!(production.contains("\"Log Out All\""));
        assert!(production.contains("secondary_dialog_button_with_disabled"));
        assert!(production.contains("danger_secondary_button_with_loading"));
        assert!(production.contains("BACKGROUND"));
        assert!(production.contains("BORDER"));
        assert!(production.contains("LocalIcon::LogOut"));
        ...
        let confirm_start = production.find("fn confirm(").expect("confirm action");
        let confirm = &production[confirm_start..];
        let confirm_end = confirm
            .find("impl DialogCloseTarget")
            .expect("confirm is followed by close-target impl");
        let confirm = &confirm[..confirm_end];
        assert!(confirm.contains("settings.logout_all(cx)"));
        assert!(confirm.contains("request_dialog_close"));
    }

    #[test]
    fn confirm_logout_all_opens_the_dialog_instead_of_logging_out_immediately() {
        let source = include_str!("logout_all_dialog.rs");
        let production = &source[..source.find("#[cfg(test)]").unwrap()];
        assert!(production.contains("fn confirm_logout_all("));
        assert!(production.contains("window.open_dialog"));
        assert!(production.contains("LogoutAllDialog::open("));

        let entry_start = production
            .find("fn confirm_logout_all(")
            .expect("confirm_logout_all entry");
        let entry = &production[entry_start..];
        let entry_end = entry
            .find("impl LogoutAllDialog")
            .expect("dialog impl follows confirm_logout_all");
        let entry = &entry[..entry_end];
        assert!(!entry.contains("logout_all(cx)"));
    }
}
