...
mod tests {
    ...
    #[test]
    fn murglar_heading_does_not_render_a_refresh_action() {
        let source = include_str!("murglar_panel.rs");
        let refresh_action = ["murglar", "refresh"].join("-");
        assert!(!source.contains(&refresh_action));
    }

    #[test]
    fn murglar_profile_card_title_and_logout_sit_together() {
        let source = include_str!("murglar_panel.rs");
        let production = &source[..source.find("#[cfg(test)]").unwrap()];
        assert!(production.contains(".child(\"Murglar profile\")"));
        assert!(!production.contains("Active Murglar profile"));
        assert_eq!(production.matches("\"murglar-logout\"").count(), 1);

        let profile_start = production
            .find(".when_some(profile, |this, profile| {")
            .expect("signed-in profile card");
        let profile_block = &production[profile_start..];
        let profile_end = profile_block
            .find(".when(signed_in && extras_loading")
            .expect("profile block should be followed by extras loading");
        let profile_block = &profile_block[..profile_end];
        assert!(profile_block.contains("profile_card(&profile)"));
        assert!(profile_block.contains("murglar_logout_button("));

        let loading_start = production
            .find(".when(signed_in && loading, |this| {")
            .expect("loading fallback");
        let loading_block = &production[loading_start..profile_start];
        assert!(loading_block.contains("Refreshing Murglar profile..."));
        assert!(loading_block.contains("murglar_logout_button("));
        assert!(loading_block.contains("!has_profile"));

        let saved_start = production
            .find(".when(signed_in && !loading && !has_profile, |this| {")
            .expect("saved session fallback");
        let saved_block = &production[saved_start..];
        assert!(saved_block.contains("Saved Murglar session"));
        assert!(saved_block.contains("murglar_logout_button("));
        assert_eq!(
            production
                .matches("murglar_logout_button(loading, cx)")
                .count(),
            3
        );
    }
    ...
    #[test]
    fn murglar_status_button_matches_player_bar_control() {
        let source = include_str!("murglar_panel.rs");
        assert!(source.contains("fn murglar_status_button("));
        assert!(source.contains(".size(px(34.))"));
        assert!(source.contains(".mr(px(4.))"));
        assert!(source.contains(".items_center()"));
        assert!(source.contains("LocalIcon::Signal"));
        assert!(source.contains("Open Murglar service status"));
        assert!(source.contains("refresh_murglar_referral"));
        assert!(source.contains("complete_referral"));
    }
}
