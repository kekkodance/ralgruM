...
active subscriptions and other runtime effects. Only provide `open_ui` when
the integration exposes its own interface and `open_settings` when it has
settings. The plugin owns those windows, dialogs, and its settings storage;
the shared `plugin_settings.json` file only tracks enabled states.
