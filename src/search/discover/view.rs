...
mod tests {
    ...
    fn discover_feed_uses_a_virtualized_list_and_discover_card_renderer() {
        ...
        // Rows measure their natural heights inside a measured-all list:
        // no uniform row pin and no fixed-height scrollbar adapter.
        assert!(!implementation.contains("pins_height"));
        assert!(!implementation.contains("FixedListScrollHandle"));
    ...
    fn loading_headings_reserve_the_subtitle_line_for_every_provider() {
        // Skeleton headings mirror the real two-line heading geometry for
        // both providers so loading rows measure their natural height.
    ...
    fn section_headings_render_the_subtitle_line_only_with_a_subtitle() {
        // With a subtitle the heading measures the full two-line slot;
        // without one the slot is absent entirely, so the title line sits
        // the normal heading gap above the carousel and the row measures
