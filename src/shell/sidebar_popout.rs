...
static POPOUT_WINDOW: LazyLock<Mutex<Option<WindowHandle<SidebarPopoutRoot>>>> =
    LazyLock::new(|| Mutex::new(None));

/// Result of a popout lifecycle request.
enum PopoutSlot {
    Occupied(WindowHandle<SidebarPopoutRoot>),
    Empty,
}
...
fn clear_slot(handle: &WindowHandle<SidebarPopoutRoot>) {
...
pub(super) fn forget_closed_popout(handle: &WindowHandle<SidebarPopoutRoot>) {
    clear_slot(handle);
}
