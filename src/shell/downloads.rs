...
mod tests {
    ...
    #[test]
    fn downloads_content_wrapper_keeps_top_and_horizontal_padding_with_inner_bottom_padding() {
        let implementation = include_str!("downloads.rs")
            .split_once("pub(super) fn render_downloads")
            .map_or_else(
                || panic!("downloads renderer must have an implementation section"),
                |(_, implementation)| implementation,
            );
        let start = implementation
            .find(".id(\"downloads-page-content\")")
            .unwrap_or_else(|| panic!("downloads content wrapper marker is missing"));
        let first_child = implementation[start..]
            .find(".child(")
            .map(|offset| start + offset)
            .unwrap_or_else(|| panic!("downloads content wrapper end marker is missing"));
        let wrapper = &implementation[start..first_child];

        // Scroll container must not carry padding so empty or short lists have zero scroll offset.
        assert!(wrapper.contains(".track_scroll("));
        assert!(wrapper.contains(".overflow_y_scroll()"));
        assert!(!wrapper.contains(".pt("));
        assert!(!wrapper.contains(".pb("));
        assert!(!wrapper.contains(".py("));
        assert!(!wrapper.contains("main_content_inset"));

        // Inner content container carries top, horizontal, and bottom padding.
        // Bottom padding scrolls with the tail so short lists keep zero offset.
        let after_outer = first_child + ".child(".len();
        let second_child = implementation[after_outer..]
            .find(".child(")
            .map(|offset| after_outer + offset)
            .unwrap_or_else(|| panic!("downloads inner content end marker is missing"));
        let inner = &implementation[first_child..second_child];

        assert!(inner.contains(".px(px(crate::music_ui::main_content_inset(&metrics)))"));
        assert!(inner.contains(".pt(px(24.))"));
        assert!(!inner.contains(".py(px(24.))"));
        assert!(inner.contains(".pb(px(DOWNLOADS_CONTENT_BOTTOM_PADDING_PX))"));
    }

    fn downloads_implementation() -> &'static str {
        include_str!("downloads.rs")
            .split_once("#[cfg(test)]")
            .map_or_else(
                || panic!("downloads implementation section is missing"),
                |(implementation, _)| implementation,
            )
    }

    fn action_cell_source() -> &'static str {
        let implementation = downloads_implementation();
        let action_start = implementation
            .find("fn action_cell(")
            .unwrap_or_else(|| panic!("downloads action cell is missing"));
        let action_end = implementation[action_start..]
            .find("\nfn icon_action_button(")
            .map(|offset| action_start + offset)
            .unwrap_or_else(|| panic!("downloads action cell end marker is missing"));
        &implementation[action_start..action_end]
    }

    #[test]
    fn download_action_cell_has_no_empty_spacer() {
        let implementation = downloads_implementation();
        let action = action_cell_source();

        // Non actionable rows return None so the flex row keeps no trailing gap.
        assert!(action.contains("-> Option<AnyElement>"));
        assert!(action.contains("DownloadStatus::Cancelled => None"));
        let helper_marker = ["fn ", "disabled_action("].concat();
        assert!(!action.contains("disabled_action"));
        assert!(!implementation.contains(&helper_marker));
        let spacer_marker = [".w(px(30.)).flex_none().into_any_element()"].concat();
        assert!(!action.contains(&spacer_marker));

        let job_start = implementation
            .find("fn render_job(")
            .unwrap_or_else(|| panic!("downloads job renderer is missing"));
        let job_end = implementation[job_start..]
            .find("\nfn job_artwork(")
            .map(|offset| job_start + offset)
            .unwrap_or_else(|| panic!("downloads job renderer end marker is missing"));
        let job = &implementation[job_start..job_end];
        assert!(job.contains(".children(action_cell("));
    }

    #[test]
    fn in_progress_downloads_show_cancel_in_action_cell() {
        let action = action_cell_source();
        assert!(action.contains("DownloadStatus::Queued"));
        assert!(action.contains("DownloadStatus::Resolving"));
        assert!(action.contains("DownloadStatus::Downloading { .. }"));
        assert!(action.contains("Cancel queued download"));
        assert!(action.contains("Cancel download"));
        assert!(action.contains("LocalIcon::X"));
        assert!(action.contains("model.cancel(id, cx)"));
    }

    #[test]
    fn failed_downloads_show_retry_in_action_cell() {
        let action = action_cell_source();
        assert!(action.contains("DownloadStatus::Failed(_)"));
        assert!(action.contains("LocalIcon::RotateRight"));
        assert!(action.contains("Retry download"));
        assert!(action.contains("model.retry(id, cx)"));
    }

    #[test]
    fn download_model_exposes_retry() {
        let model = include_str!("../downloads/model.rs");
        assert!(model.contains("pub(crate) fn retry("));
        assert!(model.contains("fn retry(&mut self, id: u64, cx: &mut Context<Self>)"));
    }

    #[test]
    fn download_action_icons_share_the_same_rendered_box() {
        assert_eq!(super::DOWNLOAD_ACTION_ICON_PX, 13.);
        let action = action_cell_source();
        let width = ".w(px(DOWNLOAD_ACTION_ICON_PX))";
        let height = ".h(px(DOWNLOAD_ACTION_ICON_PX))";
        assert_eq!(
            action.matches(width).count(),
            3,
            "FolderOpen, X, and RotateRight must share DOWNLOAD_ACTION_ICON_PX width",
        );
        assert_eq!(
            action.matches(height).count(),
            3,
            "FolderOpen, X, and RotateRight must share DOWNLOAD_ACTION_ICON_PX height",
        );
        for icon in [
            "LocalIcon::FolderOpen",
            "LocalIcon::X",
            ...
            let start = action
                .find(icon)
                .unwrap_or_else(|| panic!("{icon} is missing from download action cell"));
            let snippet = &action[start..start.saturating_add(280).min(action.len())];
            assert!(
                snippet.contains(width) && snippet.contains(height),
                "{icon} must use the shared DOWNLOAD_ACTION_ICON_PX width and height",
            );
            assert!(
                !snippet.contains(".size(px("),
                "{icon} must not use independent .size() that ignores the shared box",
            );
        }
    }
}
