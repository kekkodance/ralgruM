use gpui::{
    ...
    FontWeight, HighlightStyle, ImageSource, InteractiveText, Pixels, ScrollHandle,
    ScrollWheelEvent, SharedString, StyledText, TextAlign, UnderlineStyle, Window, canvas, div,
    fill, point, prelude::*, px, rgb, rgba, size, svg,
};
use std::{
    cell::Cell,
    collections::HashMap,
...
mod tests {
    ...
    #[test]
    fn collection_cards_render_the_subtitle_row_only_when_it_carries_content() {
        ...
        assert!(card.contains("let has_subtitle_row = show_subtitle || is_private.is_some();"));
        assert!(card.contains(".when(has_subtitle_row, |this|"));
    }
